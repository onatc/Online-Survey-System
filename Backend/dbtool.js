...
async function listUsers() {
  ...
  for (let page = 0; ; page++) {
    ...
    for (let user of users) {
      console.log(sprintf('%3d | %10s | %5d', user.id, user.username, user.admin));
    }
    if (users.length < pgSize)
      break;
  }
}
...
async function populateDatabase(adminPassword)
{
  ...
  const questions = [
    ...
    },
    {
      ...
      choices: ['Superman', 'Wonder Woman', 'Asterix', 'Black Panther', 'Captain America', 'Catwoman']
    },
    {
      question: "What is your favorite maxflow algorithm?",
      choices: ['Dinic', 'Ford-Fulkerson', 'Edmonds-Karp', 'Ahuja-Orlin ISAP', "Goldberg's Push-Relabel"]
    },
    {
    ...
    },
    {
      ...
      choices: ['TON 618', 'IC 1101', 'S5 0014+81', 'SDSS J102325.31+514251.0', 'NGC 6166']
    },
    {
  ...
  const sampleQuestions = questions.map(q => axios({
      method: 'POST',
